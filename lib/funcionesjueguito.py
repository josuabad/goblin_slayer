...
def combate(inventario, stats_player, enemigo_selec):  # Esta función comprueba que combate debe activar
...
def combate_estafermo(inventario, stats_player):
    ...
    Combate = True  # La variableCombate indica que una vez se hayan creado las variables que afectan al enemigo
    # comienza
    while Combate == True:
        ...
        while turno_player == True:  # Bucle que representa el turno del jugador
            print("Has impactado")
            dam_player = (stats_player.get("destreza") - 10) + dados(
                4)  # Calcula el daño del jugador en base al arma, que en este caso al ser el puño he puesto
            # directamente en dados(4), pero deberia ser el valor del arma que lleve en ese momento
            ...
            if vida_estafermo <= 0:  # Si la vida esta por debajo de 0 se termina el combate
                ...
                Combate, turno_player = False, False  # Se cierra el turno y el combate al mismo tiempo para evitar
                # problemas
                break
            print(f"El Estafermo tiene {vida_estafermo} de vida.")
            decision = input(
                "¿Quieres seguir luchando?  SI||NO: ")  # Pregunta al jugador despues de cada golpe si quiere seguir
            # jugando
            ...
            if decision == "NO":
                ...
                pass
            turno_player, turno_enemigo = False, True  # Al igual que antes el turno del jugador se desactiva y el
            # del enemigo se activa al mismo instante para evitar problemas con los bucles

        while turno_enemigo == True:
            print("\nEs el turno del Estafermo")
            impacto_player = 12 + (stats_player.get(
                "destreza") - 10)  # Deberemos calcular cuanto debe sacar el enemigo para impactar al jugador,
            # esto en base a la estadistica de destreza
            ...
            if golpe_estafermo >= impacto_player:  # El enemigo golpea al jugador
                ...
                vida_player = stats_player.get(
                    "vida")  # Creamos una variable con el valor de vida que en ese momento tiene el jugador
                vida_player = vida_player - dam_estafermo  # Actualizamos el valor de la vida del jugador
                stats_player.update(
                    {"vida": vida_player})  # Subimos, por asñi decirlo, el nuevo valor al diccionario del jugador
                print(
                    f"El Estafermo te ha hecho {dam_estafermo} punto de daño tienes, {vida_player} puntos de vida")
                # Imprimimos detalles del combate como el daño y nuestra vida
...
def combate_ratas(inventario, stats_player):
    ...
    while Combate == True:
        ...
        while turno_player == True:
            print("Has impactado")
            dam_player = (stats_player.get("destreza") - 10) + dados(4)  # Calcula el daño del jugador en base al
            # arma, que en este caso al ser el puño he puesto directamente en dados(4), pero debería ser el valor del
            # arma que lleve en ese momento
            ...
            if vida_rata <= 0:
                ...
                if decision == 1:
                    ...
                    if int_huida >= huida:
                        ...
                        Combate, turno_player = False, False  # Se cierra el turno y el combate al mismo tiempo para
                        # evitar problemas
                ...
                turno_player, turno_enemigo = False, True  # Al igual que antes el turno del jugador se desactiva y
                # el del enemigo se activa al mismo instante para evitar problemas con los bucles
        while turno_enemigo == True:
            ...
            if golpe_rata >= impacto_player:  # El enemigo golpea al jugador
                ...
                stats_player.update(
                    {"vida": vida_player})  # Subimos, por asñi decirlo, el nuevo valor al diccionario del jugador
                print(
                    f"La rata te ha hecho {dam_rata} punto de daño tienes, {vida_player} puntos de vida")  #
                # Imprimimos detalles del combate como el daño y nuestra vida
                turno_player, turno_enemigo = True, False  # Se activan y desactivan los turnos homogeneamente
            else:  # El estafermo no ha impactado
